...
%% Plot the schedule
% We visualize the schedule and plot both the water and oil 
% using a simple supplied volume renderer. At the same time
% we visualize the sum of the oil/water ratio in the producer for
% without polymer injection in a single pie chart.
...
nDigits = floor(log10(numel(statesOW) - 1)) + 1;

figure(3); clf;
for i = 1 : numel(statesOW) - 1,
...
% the non-polymer run. The result shows that
...
legend({'Water without polymer',...
       'Oil without polymer'}, 'Location', 'NorthEast');
ylabel('stb');
xlabel('Years');
