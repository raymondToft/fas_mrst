...
diary '4_Cycles_bonus_pc_0.txt'
fprintf('This is the testing results from running both cycletypes and Newton for higher grid sizes');
% 
for i= 1:5
    
%Test A 
[nx,ny,nz] = deal( 72,  72, 4);
[Dx,Dy,Dz] = deal(500, 500, 50);
newModel.grid = cartGrid([nx, ny, nz], [Dx, Dy, Dz]);
newModel.grid = computeGeometry(newModel.grid);

newModel.homogeneous='spe10';
p = gaussianField(newModel.grid.cartDims, perm_range, gauss_filter_size, std);
        K = p.^3. * (1e-5)^2./(0.81 * 72 * (1-p).^2);
        rock = makeRock(newModel.grid, K(:), p (:));
newModel.perm = rock.perm;
newModel.poro = rock.poro;

newModel.cycle_type = 'F_cycle';
fprintf('\n FAS Test A: F-cycle 72*72*4 \n');
fprintf(' RunTime, Residual, Iterations \n');
result_A_FAS_F = runSumulationFAS(newModel, constraints, nSimulations);

newModel.cycle_type = 'V_cycle';

fprintf('\n FAS Test A: V-cycle 72*72*4 \n');
fprintf(' RunTime, Residual, Iterations \n');
result_A_FAS_V = runSumulationFAS(newModel, constraints, nSimulations);

fprintf('\n Newton Test A: V-cycle 72*72*4 \n');
fprintf(' RunTime, Residual, Iterations \n');
result_A_N = runSumulationNewton(newModel, constraints, nSimulations);

%Test B 

[nx,ny,nz] = deal( 80,  80, 4);
[Dx,Dy,Dz] = deal(500, 500, 50);
newModel.grid = cartGrid([nx, ny, nz], [Dx, Dy, Dz]);
newModel.grid = computeGeometry(grid);
...
cycle_type = 'F_cycle';
fprintf('\n FAS Test B: F-cycle 80*80*4\n');
fprintf(' RunTime, Residual, Iterations \n');
result_B_FAS_F = runSumulationFAS(newModel, constraints, nSimulations);

cycle_type = 'V_cycle';
fprintf('\n Newton Test B: V-cycle 80*80*4\n');
fprintf(' RunTime, Residual, Iterations \n');
result_B_FAS_V = runSumulationFAS(newModel, constraints, nSimulations);

fprintf('\n Newton Test B: Newton 80*80*4\n');
fprintf(' RunTime, Residual, Iterations \n');
