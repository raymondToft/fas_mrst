...
% % %% Define rock model
%  homogeneous = 'true';
%  if(strcmp(homogeneous, 'true'))
%     rock = makeRock(G, 30*milli*darcy, 0.25);
% else
%     perm_range = [0.2 0.4];
%     filter_size = [3 3 3];
%     std = 2.5;
%      p = gaussianField(G.cartDims, perm_range, filter_size, std);
%      K = p.^3. * (1e-5)^2./(0.81 * 72 * (1-p).^2);
%      rock = makeRock(G, K(:), p (:));
% end

% SPE10
[G, W, rock] = getSPE10setup(4);
mp = 0.1;
...
maxits   = 1000;                  % max number of Newton its
...
while t < totTime
   ...
   while (resNorm > tol) && (nit <= maxits)
    ...
    % Scale equations by dt over pore volume so that they are unit-sized
    % (i.e. residual corresponds to saturation error, which is 1 for large
    % values).
      oil = oil.*(dt./pv_r);
      water = water.*(dt./pv_r);
      eqs = {oil, water};
...
for i = 1:numSteps
    ...
    axis off;
    subplot(1, 2, 1)
    plotCellData(G, sol(20).pressure, 'edgecolor', 'none');
    colorbar (); axis equal tight ; view (3);
    title('Pressure')
%     axis off;
    axis equal tight
    view(0, 90);
    subplot(1, 2, 2)
    plotCellData(G, sol(20).s, 'edgecolor', 'none');
    colorbar (); axis equal tight ; view (3);
%     axis off;
    caxis([0, 1])
    view(0, 90);
